 ...
 */
Controller.prototype.synchronize =
  Controller.prototype.sync = function (data, job) {
...
Controller.prototype.notify = function (name, data) {
  return new B(function (resolve, reject) {
    ...
    job.save(function (e) {
      if (e) {
        debug('failed to save job "%s": %s', name, e.message);
        reject(e);
...
Controller.prototype.process = function (job, done, ctx) {
  ...
  report(job, [
    'processing "%s" job #%d with %j...', job.type, job.id, job.data
  ]);
