...
Controller.prototype.subscribe = function (data, job) {
  ...
  var subscription = (data instanceof Subscription)
    ? data
    : new Subscription(Subscription.restrict(data));
    ...
    .filter(function (s) {
      ...
      return sync
        ...
        .catch(function (error) {
          report(job, [
              'failed to sync subscription "%s": %s',
              s.id,
              error.message
          ]);
 ...
 * @param {Number|Date} [delay]
 *
 * @return {Promise<Kue.Job>}
 */
Controller.prototype.notify = function (name, data, delay) {
...
Controller.prototype.start = function (jobs) {
  ...
  subscriptions
    .finally(function resume() {
      ...
      debug('start promoting delayed jobs...');
      q.jobs.promote(self.options.promote);
