...
function Controller(options) {
  ...
  this.listener = new Listener()
    ...
    .on('error', function (error) {
      debug('uncaught listener error: %s', error.message);
      debug('%s', error.stack);
    });
...
Controller.prototype.start = function (jobs) {
  ...
  var subscriptions = this.sync({ all: true });
  ...
  if (this.options.listen) {
    listener
      ...
      .once('connected', function () {
        ...
        subscriptions
          .then(function (s) {
            if (s.length) return listener.add(s);
          })

          .catch(function (error) {
            debug('unable to listen for updates: %s', error.message);
          });
      });
