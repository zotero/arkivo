...
function connect(name) {
  debug('connecting to redis at %s:%d...', config.host, config.port);
...
function Transaction(database) {
  debug('new transaction for "%s" namespace', database.name);

  Proxy.call(this, database.name, 'multi');
  this.multi = database.client.multi();
}
...
function Proxy(name, dest, dsuffix) {
   ...
   *
   *
   * @return The return value of the proxy's target, or
  ...
  this.namespace = function namespace(cmd, arity, args, suffix) {
    ...
    debug('%s: %s with %d argument(s)',
      this.name, cmd.toUpperCase(), args.length);
