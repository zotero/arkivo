...
api.route('/subscription')
  ...
  .post(body.json(), function (req, res, next) {
    controller
      .subscribe(extend(req.query, req.body))
...
api.route('/subscription/:subscription')
  ...
  .delete(function (req, res, next) {
    controller
      .unsubscribe(req.subscription)

      .then(function () {
        if (req.query['invalidate-key'])
          return req.subscription.invalidate();
      })
...
api.use(function (error, req, res) {
  res
    .status(code(error))
...
/* @return {Number} an HTTP status code for error. */
function code(error) {
...
/* @return {Object} The link header object for the range. */
