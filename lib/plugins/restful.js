...
// --- Dependencies ---
var debug = require('debug')('arkivo:plugins:restful');
...
module.exports = {
  ...
  process: co(function* (sync) {
    ...
    yield B.map(sync.deleted, client.delete.bind(client));

    debug('[%s] processed %d deleted item(s)', id, sync.deleted.length);

    yield B.map(sync.created.map(lookup), client.create.bind(client));

    debug('[%s] processed %d created item(s)', id, sync.created.length);

    yield B.map(sync.updated.map(lookup), client.update.bind(client));
