...
Subscription.validateItemsUrl = function (url, key) {
  return new B(function (resolve, reject) {
    if (!(/^(\/?(users|groups)\/\d+\/?(publications\/items|items))/)
        .test(url)) {
...
/**
 * Lookup a Zotero item's external id for a given plugin.
 *
 * @return {Promise<String>}
 */
Subscription.lookup = function (library, plugin, key) {
  return this.db.hget([plugin, library].join(':'), key);
};

/**
 * Remember a Zotero item's external id for a given plugin.
 *
 * @return {Promise<String>}
 */
Subscription.remember = function (library, plugin, key, id) {
...
Subscription.prototype.lookup = function (plugin, key) {
  return Subscription.lookup(this.library, plugin, key);
};

Subscription.prototype.remember = function (plugin, key, id) {
