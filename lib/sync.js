...
Session.prototype.download = co(function* (keys) {
  ...
  var i, ii, j, jj, batch, item, key, message, items;
  ...
  for (i = 0, ii = keys.length; i < ii; ++i) {
    ...
    debug('%s: %d items received...', s.id, items.length);

    for (j = 0, jj = items.length; j < jj; ++j) {
      yield this.register(items[j], keys);
    }
  }

  debug('%s: downloaded %d item(s)', s.id, keys.length);

  return this;
});

Session.prototype.register = co(function* (item, keys) {
  assert(item);
  ...
  // parent item's data!
  if ((pkey = parent(item)))
    if (keys.indexOf(pkey) === -1) {
      debug('%s: requesting additional parent item...', this.subscription.id);
      keys.push(pkey);
    }
