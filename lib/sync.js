...
Synchronizer.prototype.process = function (ids, job) {
  var self = this, subscriptions, progress;

  ids = compact(ids || []);

  if (ids.length) {
    subscriptions = B.map(ids, function (id) {
      report(job, ['loading subscription "%s"...', id]);
...
Synchronizer.prototype.listen = function () {
  debug('start listening for sync requests...');

  q.jobs
    .process('sync', defaults.workers, this.onSync.bind(this));
