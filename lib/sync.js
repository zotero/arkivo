...
Synchronization.prototype.request = co(function* () {
  ...
  debug('%s: initial response received for version %d (was %d)',
  ...
  debug('%s: full response received');
...
Synchronization.prototype.process = co(function* () {
  assert(this.data, 'no sync data to process!');

  debug('%s: processing %d items...',
      this.subscription.id, this.data.length);
...
Synchronization.prototype.finalize = co(function* () {
  if (this.version)
    this.subscription.version = this.version;
